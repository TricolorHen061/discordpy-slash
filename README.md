...
```
In the above example, we made a `ping` command that has a button that says "Got it!". When the button is clicked, the `button_clicked` function is given two parameters: a `slash_context` object and a list of dictionaries containing info about the buttons. You can modify the dictionaries to be whatever you want. You must return it for it to get applied to the origial message. You don't have to return anything if you don't want the message to be changed. In the example above, the button is disabled. Notice the `button_functions` parameter on the `sync_all_commands` function. This needs to be a list of functions the library has access to.  
